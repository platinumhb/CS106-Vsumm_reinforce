...
if __name__ == '__main__':
    ...
    # h5_res = list(h5_res.keys())[args.idx]
    ...
    h5_res = h5py.File(args.path, 'r')  # Mở tệp HDF5
    keys_list = list(h5_res.keys())  # Chuyển các keys thành danh sách
    key = keys_list[args.idx]  # Lấy key từ danh sách dựa trên chỉ số
    summary = h5_res[key]['machine_summary'][...]  # Truy cập vào machine_summary dựa trên key
    h5_res.close()  # Đóng tệp HDF5
    frm2video(args.frm_dir, summary, vid_writer)  # Tạo video từ summary
    vid_writer.release()  # Giải phóng đối tượng video writer